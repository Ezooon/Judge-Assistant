...
class MDBackdrop(ThemableBehavior, MDFloatLayout):
    ...
    def open(self, open_up_to: int = 0, open_open=False) -> None:
        """
        ...
        """
        if open_up_to == 0:
            self.close(False, dp(60))
            self.dispatch("on_open")
            self._front_layer_open = True
            return

        # self.animate_opacity_icon()
        if self._front_layer_open and not open_open:
            self.close()
            return
        ...
        if y != self.ids.header_button.height - self.ids._front_layer.height:
            self._front_layer_open = True
        self.dispatch("on_open")

    def close(self, allthewaydown=False, keepshowing=0) -> None:
        """Opens the front layer."""
        # if not self._front_layer_open and not allthewaydown:
        #     return
        if allthewaydown:
            y = - (self.ids._front_layer.height + self.ids.header_button.height)
        else:
            y = self.ids.header_button.height - self.ids._front_layer.height + keepshowing

        Animation(y=y, d=self.closing_time, t=self.closing_transition).start(
            self.ids._front_layer
        )
    ...
    def add_widget(self, widget, index=0, canvas=None):
        if widget.__class__ in (MDBackdropToolbar, _BackLayer, _FrontLayer, BoxLayout):
    ...
    def on_touch_down(self, touch):
        super(MDBackdrop, self).on_touch_down(touch)
        if touch.pos[1] > self.ids._front_layer.top:
...
class MDBackdropToolbar(MDTopAppBar):
    ...
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        self.theme_cls.unbind(primary_palette=self.update_md_bg_color)
...
class MDBackdropBackLayer(MDFloatLayout):
