...
class ButtonContentsIcon:
    ...
    def __init__(self, **kwargs):
        ...
        if self.user_font_size:
            self.icon_size = sp(20)  # self.user_font_size
...
class MDFloatingActionButtonSpeedDial(ThemableBehavior, FloatLayout):
    ...
    all_set = ObjectProperty(lambda x: None)
    ...
    def get_button(self, icon_name):
        for child in self.children:
            if isinstance(child, MDFloatingBottomButton):
                if child.icon == icon_name:
                    return child
